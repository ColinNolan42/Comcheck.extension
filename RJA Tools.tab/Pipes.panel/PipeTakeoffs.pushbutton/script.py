...
Fixture type is selected from the ComboBox in the Pipes panel (ui.py).
Pipe size and AFF are set automatically based on fixture selection.
...
# ============================================================================
# FIXTURE SETTINGS READER
# ============================================================================
def get_fixture_settings():
    """Read the current fixture envvar and return (fixture_name, dia_ft, aff_ft).

    Falls back to DEFAULT_FIXTURE if the envvar is missing or unrecognized.
    """
    fixture_name = script.get_envvar(ENVVAR_FIXTURE) or DEFAULT_FIXTURE
    if fixture_name not in FIXTURES:
        fixture_name = DEFAULT_FIXTURE

    dia_in, aff_in = FIXTURES[fixture_name]
    return fixture_name, dia_in / 12.0, aff_in / 12.0
...
def build_takeoff(main_pipe, click1, click2, branch_dia_ft, aff_height):
    ...
    logger.debug(
        "Takeoff complete: tee + 4 pipes + 3 elbows ({:.3f} ft branch on {} main)"
        .format(branch_dia_ft, props['pipe_type_name'])
    )
...
def main():
    ...
    try:
        while True:
            try:
                # Read fixture from the ribbon combobox envvar (set by ui.py)
                fixture_name, branch_dia_ft, aff_height_ft = get_fixture_settings()

                # Click 1: pick main pipe
                ref = uidoc.Selection.PickObject(
                    ObjectType.Element,
                    pipe_filter,
                    "Pick a CW/HW/HWC main pipe  |  Fixture: {}  "
                    "(ESC to exit)".format(fixture_name)
                )
                ...
                click2 = uidoc.Selection.PickPoint(
                    "Pick destination point  |  Fixture: {}  "
                    "(ESC to cancel)".format(fixture_name)
                )
